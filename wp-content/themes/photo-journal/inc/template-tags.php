...
function photo_journal_footer_content() {
	...
	$default_footer_content = sprintf( _x( 'Copyright &copy; %1$s %2$s. All Rights Reserved. %3$s ', '1: Year, 2: Site Title with home URL, 3: Privacy Policy Link', 'photo-journal' ), esc_html( date_i18n( __( 'Y', 'photo-journal' ) ) ), '<a href="'. esc_url( home_url( '/' ) ) .'">'. esc_html( get_bloginfo( 'name', 'display' ) ) . '</a>', get_the_privacy_policy_link() . $theme_data->get( 'Name' ) . '&nbsp;' . esc_html__( 'by', 'photo-journal' ) . '&nbsp;<a target="_blank" href="' . $theme_data->get( 'AuthorURI' ) . '">' . esc_html( $theme_data->get( 'Author' ) ) . '</a>' );
